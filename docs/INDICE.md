# Índice de documentación

Lee en este orden si eres nuevo en el proyecto:

| Orden | Documento | Para qué sirve |
|------:|-----------|----------------|
| 1 | [README.md](../README.md) | Qué es el proyecto, cómo levantarlo en 5 minutos |
| 2 | [COMO_FUNCIONA.md](COMO_FUNCIONA.md) | Arquitectura, capas, flujo de arranque, mapa mental |
| 3 | [NEGOCIO.md](NEGOCIO.md) | Reglas de producto: dinero, soft-delete, transferencias |
| 4 | [MODELOS.md](MODELOS.md) | Tablas, columnas, relaciones, migraciones |
| 5 | [API.md](API.md) | Catálogo completo de endpoints HTTP |
| 5b | [FRONTEND.md](FRONTEND.md) | Primer día FE: auth, demo users, dashboard, Postman |
| 6 | [SEGURIDAD.md](SEGURIDAD.md) | Auth, JWT, MFA, OWASP, variables de entorno |
| 7 | [REPOSITORIOS.md](REPOSITORIOS.md) | Capa de acceso a datos |
| 8 | [TESTING.md](TESTING.md) | Cómo y por qué testamos |
| 9 | [HOJA_RUTA.md](HOJA_RUTA.md) | Historial de maduración (pasos 1–11) |

## ¿Qué pregunta tienes?

| Pregunta | Documento |
|----------|-----------|
| ¿Cómo enciendo el proyecto? | README → Inicio rápido |
| ¿Por qué hay tantas carpetas en `app/`? | COMO_FUNCIONA |
| ¿El saldo se puede editar a mano? | NEGOCIO |
| ¿Qué pasa si hago DELETE? | NEGOCIO + API |
| ¿Cómo me autentico / soy admin? | SEGURIDAD + API (auth) |
| ¿Qué URL llamo desde el frontend? | FRONTEND + API |
| ¿Cómo se ve la base de datos? | MODELOS |
| ¿Dónde escribo un SELECT? | REPOSITORIOS |
| ¿Cómo corro los tests? | TESTING |
| ¿Qué se hizo en cada etapa? | HOJA_RUTA |
